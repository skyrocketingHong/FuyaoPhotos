...
采用 FuyaoLocale / FuyaoColorPicker 的 Material 3 基线：ColorPicker 同款 48dp 窄顶栏，搭配半粗标题、28dp 操作图标和 48dp 点击区域，保留动态色与统一分组。顶栏按实际状态栏高度和横向刘海避让，不增加额外上下留白。照片为主、参数为辅助面板；窄屏上下排列，中宽窗口采用两栏。镜头配置为独立编辑页，支持数字键盘、就地验证和删除撤销。
