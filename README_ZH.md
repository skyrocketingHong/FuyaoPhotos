...
当前本机工程使用 macOS 的 SF Compact Rounded（字重 500，启用 `cv04` 居中冒号、`cv05` 大写 I 与 `pnum` 比例数字），数字 `1` 保留在该字体中；Terminal 的 SF Mono Medium 用于 `0`、`2`～`9`。这是依据截图调整的复刻方案，不代表已经确认 Apple 原片字库。空格、冒号、小数点、斜线和括号使用比例字体，不启用带衬线的数字 1 替代字形；比例字体按等宽字体的实际大写轮廓高度校正，字母与窄数字 `1` 不再只依赖相同的名义字号。测量、换行、预览、导出及 HDR 文字覆盖共用该校正和同一排版路径。基于此目录编译的 APK 会包含可用的本地字体。字体二进制与原始参考图目录被 Git 忽略；选入 README 的图片保存在 `assets/readme/` 并纳入版本控制，图片权利独立于源码许可证；不含这些字体的源码检出仍可使用系统比例字形与等宽字形混排构建。运行时导入的字体最大 10 MB，仅存放在应用私有目录。
