...
- 编辑首页退出以及设置、镜头页面的返回均先确认；取消手势或确认后继续编辑时保留草稿。确认退出后清理本次临时副本与草稿，已保存到相册的照片保留。保存提示自动消失，也可手动关闭。
...
设置、镜头列表、镜头编辑和全屏预览共用 Navigation Compose 返回栈，处理预测性返回进度及取消；有打开的照片时，编辑器根页面先确认再退出；空白首页交由 Android 返回桌面。设置和镜头页面返回时同样确认，明确点击保存后无需再次确认放弃修改。导出选项使用 Material 3 底部面板、分段格式选择及整行可点击的元数据开关。实际手势效果仍需真机验收。
...
三条配置的机型均使用应用读到的原始 EXIF 名称，例如 `Xiaomi 17 Ultra by Leica`。物理焦距只填 Camera2／EXIF 提供的实际毫米数，不填 35mm 等效焦距；未知时留空。光圈和输出像素数由每张照片读取。
