...
1. 打开一张照片，或一次选择多张。HDR/动态照片建议使用“从文件导入（原片）”。
2. 修改拍摄信息和卡片样式；多图可横向切换编辑。
3. 点击“保存”，选择格式及 JPEG 质量，保存本次全部照片。
...
| 支持的 JPEG 动态照片 / Microvideo | JPEG | 原视频、音频及视频元数据 |
| 支持的 JPEG Ultra HDR 动态照片（Android 14+） | JPEG | HDR 与动态数据 |

JPEG 底图和增益图会重新编码，视频不转码。HDR/动态照片不能导出为 PNG。请导入完整原片，聊天应用或文件提供程序已移除的数据无法恢复。
...
- 可选保留的拍摄参数不包含静态图 GPS、序列号、MakerNote、XMP 和缩略图。原视频元数据保持不变，其中可能包含位置；卡片上的姓名和地点仍会显示在导出图片中。
