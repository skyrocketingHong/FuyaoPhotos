...
应用导出的香港海滨照片，右下角叠加拍摄信息卡，尺寸为 **4080 × 3072**。此样例采用整段等宽字体，当前默认样式为圆体字母与等宽数字混排。点击图片可查看原尺寸文件。
...
- 设备与署名使用暖黄色，拍摄参数使用白色。圆体字母与等宽数字保持不透明，背景为模糊后的照片叠加半透明中性灰。
...
- 默认采用混排参考样式：本地字体可用时，字母与标点使用 SF Pro Rounded Medium，并启用带上下横线的大写 I；数字使用 SF Mono Medium。缺失时回退系统无衬线字母与等宽数字。导入 TTF/OTF/TTC 后整段统一使用该字体，重置恢复默认混排。
...
当前本机工程使用 macOS 的 SF Pro Rounded（字重 500，启用 `cv05` 大写 I 替代字形）与 Terminal 的 SF Mono Medium 混排，后者用于 ASCII 数字。这是依据截图调整的复刻方案，不代表已经确认 Apple 原片字库。空格、冒号、小数点、斜线和括号使用比例字体；测量、换行、预览、导出及 HDR 文字覆盖共用同一排版路径。基于此目录编译的 APK 会包含可用的本地字体。字体二进制与原始参考图目录被 Git 忽略；选入 README 的图片保存在 `assets/readme/` 并纳入版本控制，图片权利独立于源码许可证；不含这些字体的源码检出仍可使用系统无衬线字母和等宽数字构建。运行时导入的字体最大 10 MB，仅存放在应用私有目录。
