...
应用导出的香港海滨照片，右下角叠加拍摄信息卡，尺寸为 **4080 × 3072**。此样例采用整段等宽字体，当前默认样式为紧凑圆体与等宽字形混排。点击图片可查看原尺寸文件。
...
- 设备与署名使用暖黄色，拍摄参数使用白色。混排文字保持不透明，背景为模糊后的照片叠加半透明中性灰。
...
- 默认采用混排参考样式：本地字体可用时，字母、标点和比例数字 `1` 使用 SF Compact Rounded Medium，启用居中冒号与带上下横线的大写 I；`0`、`2`～`9` 使用 SF Mono Medium。缺失时回退系统比例字形与等宽字形混排。导入 TTF/OTF/TTC 后整段统一使用该字体，重置恢复默认混排。
...
全屏预览先显示编辑区的缩略图，再加载原尺寸渲染结果，并保留缩放位置。原尺寸预览和导出共用内存分配锁；加载前检查可用内存，失败时保留有明确标识的缩略预览并提供重试。工作区结合可用宽高、字号与键盘状态分配照片和编辑空间，并通过 WindowManager 将两部分避开分隔型折叠铰链。窄窗口保持上下布局；表单内容带安全区滚动，全屏照片背景延伸至系统栏。导入、保存、预览和字体错误提供恢复步骤；TalkBack 可使用命名操作切换照片。
...
当前本机工程使用 macOS 的 SF Compact Rounded（字重 500，启用 `cv04` 居中冒号、`cv05` 大写 I 与 `pnum` 比例数字），数字 `1` 保留在该字体中；Terminal 的 SF Mono Medium 用于 `0`、`2`～`9`。这是依据截图调整的复刻方案，不代表已经确认 Apple 原片字库。空格、冒号、小数点、斜线和括号使用比例字体，不启用带衬线的数字 1 替代字形；比例字体按等宽字体的实际大写轮廓高度校正，字母与窄数字 `1` 不再只依赖相同的名义字号。独立比例数字 `1` 在此基础上统一增加 3% 光学字号补偿，保持基线，并应用于所有字段。测量、换行、预览、导出及 HDR 文字覆盖共用该校正和同一排版路径。基于此目录编译的 APK 会包含可用的本地字体。字体二进制与原始参考图目录被 Git 忽略；选入 README 的图片保存在 `assets/readme/` 并纳入版本控制，图片权利独立于源码许可证；不含这些字体的源码检出仍可使用系统比例字形与等宽字形混排构建。运行时导入的字体最大 10 MB，仅存放在应用私有目录。
