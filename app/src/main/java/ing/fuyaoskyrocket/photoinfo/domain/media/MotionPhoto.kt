...
data class MediaEnvelope(val jpeg: Boolean, val hdrHint: Boolean = false, val motion: MotionVideo? = null, val blocked: Boolean = false)
...
object MotionPhoto {
    ...
    fun inspect(file:File,mime:String,embeddedXmp:String?=null):MediaEnvelope {
        ...
        return try {
            ...
            require(end+(if(auxiliary.isEmpty() && video!=null)primaryPadding else 0L)==(video?.offset ?: file.length())) { "Unrecognized trailing photo data" }
            MediaEnvelope(true,hdr,video)
