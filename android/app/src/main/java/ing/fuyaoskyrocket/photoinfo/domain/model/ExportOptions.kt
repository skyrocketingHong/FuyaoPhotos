...
data class ExportOptions(
    ...
    fun fields() = listOf(format.name, jpegQuality.toString(), keepExif.toString(), keepLocation.toString(), keepCaptureTime.toString(), separateLivePhoto.toString(), applePortrait.toString())
    companion object {
        fun restore(fields: List<String>) = runCatching {
            require(fields.size in 5..7)
            ExportOptions(ExportFormat.valueOf(fields[0]), fields[1].toInt(), fields[2].toBooleanStrict(),
                ...
                fields.getOrNull(6)?.toBooleanStrict() ?: false).sanitized()
